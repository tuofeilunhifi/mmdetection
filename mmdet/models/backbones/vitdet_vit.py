...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        # if self.modify_size:
        #     x = F.interpolate(x, size=self.grid_size, align_corners=False, mode=self.interpolate_mode)
        if self.flatten:
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 window_size=None, attn_head_dim=None, window=False, use_rel_pos_bias=False):
        super().__init__()
...
class ViTDetVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        #         if ((i + 1) % interval != 0) else self.grid_size,
        #         window=((i + 1) % interval != 0))
        #     self.blocks.append(Block(**_layer_cfg))
        self.blocks = nn.ModuleList([
            Block(
                dim=self.embed_dims, num_heads=self.num_heads, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                drop=drop_rate, attn_drop=0., drop_path=dpr[i], norm_layer=norm_layer,
                init_values=None, window_size=(14, 14) if ((i + 1) % interval != 0) else self.grid_size, window=((i + 1) % interval != 0), use_rel_pos_bias=use_rel_pos_bias)
            for i in range(self.num_layers)])
    ...
    def forward(self, x):
        ...
        #             out = F.interpolate(out, size=self.out_grid_size, align_corners=False, mode=self.interpolate_mode)
